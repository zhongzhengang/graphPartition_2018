	...
	public static class InitPartition
		...
		public void compute(
			...
			if (partition == -1) {
				...
				if (isVertexBalance) {
//					// Hash顶点划分
					partition = (short) (vertex.getId().hashCode()%(numberOfPartitions));
					aggregate(loadAggregatorNames[partition], new LongWritable(1));
				} else {
					// 边平衡
	...
	public static class VertexMigration
		...
		private short computeNewPartition(
			...
			for (short i = 0; i < numberOfPartitions ; i++) {
				...
				double PF = computeW(i); // 1*(当前分区边数/最大预设分区边数),：（0,1），越稳定越好
		...
		@Override
		public void preSuperstep() {
			balanceWeight = getContext().getConfiguration().getFloat(
					Const.BALANCE_WEIGHT, Const.DEFAULT_BALANCE_WEIGHT);
